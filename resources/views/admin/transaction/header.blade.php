						...
						<select name="message" id="message" class="form-control">
							...
							@if($messageList)
								@foreach($messageList as $ml)
									<option value="{{ $ml->message }}">{{ str_replace('_',' ', $ml->message) }}</option>
								@endforeach
							@endif
						</select>
                    </div>
...
<script>
    ...
    function data_table(page_url){
        ...
        var base_url = '{{ route("admin.transactions") }}';
        if(page_url)
        {
            base_url = page_url;
            current_url = page_url;
        ...
        $.ajax({
            ...
            type:'GET',
            dataType:'html',
            data:{skey:skey,suser:suser,message:message,sdate:sdate,"_token": "{{ csrf_token() }}"},
