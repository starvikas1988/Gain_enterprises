...
<div class="container mt-4">
    ...
    <a href="{{ route('restaurant.tables.index') }}" class="btn btn-secondary mb-3">Back to Table Management</a>

    <!-- Table Edit Form -->
    <div class="card">
        <div class="card-body">
            <form action="{{ route('restaurant.tables.update', $table->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="capacity" name="capacity" class="form-control" value="{{ old('capacity', $table->capacity) }}" min="1" required>
                    @error('capacity')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
