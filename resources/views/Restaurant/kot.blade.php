...
<style>
    ...
    $('#categorySelect').change(function () {
        ...
        if (categoryId) {
            $.ajax({
                url: "{{ route('restaurant.getProducts') }}",
                type: "POST",
                data: {
...
function updateSubtotal(input) {
    ...
    $('#openPaymentModal').click(function() {
        ...
        if (couponCode !== "") {
            $.ajax({
                url: "{{ route('coupon.verify') }}", // Laravel route
                type: "POST",
                data: { 
                    code: couponCode, 
    ...
    let restaurantId = "{{ auth()->user()->id }}"; // Get logged-in restaurant ID
    ...
    $.ajax({
        url: "{{ route('orders.place') }}", // Laravel route for saving order
        type: "POST",
        data: {
            restaurant_id: restaurantId,
