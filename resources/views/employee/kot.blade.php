...
<style>
        ...
        <div class="pos-container">
            ...
            <div class="left-panel">
                ...
                <div class="cart-footer">
                    ...
                    {{-- <button class="btn btn-primary btn-pos">UPI</button> --}}
                    <button id="openPaymentModal" class="btn btn-success btn-pos">Pay</button>
                    {{-- <button class="btn btn-info btn-pos">Pay All</button> --}}
                </div>
            </div>
            ...
            <div class="right-panel">
                ...
                <div class="mb-3">
                    ...
                    <select id="orderTypeSelect" class="form-select">
                        ...
                        <option value="{{ $orderType->name }}" data-name="{{ $orderType->name }}">{{ $orderType->name }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    $('#categorySelect').change(function () {
        ...
        if (categoryId) {
            $.ajax({
                url: "{{ route('employee.getProducts') }}",
                type: "POST",
...
function updateSubtotal(input) {
    ...
    $('#openPaymentModal').click(function() {
                ...
                $("#cart-body tr").each(function () {
                    let productId = $(this).data("product-id");
                    let restaurantId = "{{ auth()->user()->restaurant_id }}"; 
                    let qty = parseInt($(this).find(".qty-input").val());
        ...
        if (couponCode !== "") {
            $.ajax({
                url: "{{ route('employee.coupon.verify') }}", // Laravel route
                type: "POST",
                xhrFields: { withCredentials: true },
                data: { 
                    code: couponCode, 
    ...
    let restaurantId = "{{ auth()->user()->restaurant_id  }}"; // Get logged-in restaurant ID
    ...
    $.ajax({
        url: "{{ route('employee.orders.place') }}", // Laravel route for saving order
        type: "POST",
        xhrFields: { withCredentials: true },
        data: {
            restaurant_id: "{{ auth()->guard('employee')->user()->restaurant_id }}",
